/*
 * FluxGarage RoboEyes for GC9A01A Round Display V 2.0.0 (Modified for TFT_eSPI)
 * Adapted to use TFT_eSPI library for smooth animation without flickering using sprites.
 * Original modifications by Paskal19853d, ported to TFT_eSPI by Grok.
*/
#ifndef _FLUXGARAGE_ROBOEYES_GC9A01A_TFT_ESPI_H
#define _FLUXGARAGE_ROBOEYES_GC9A01A_TFT_ESPI_H

#include <TFT_eSPI.h>
#include <TFT_eSprite.h>

// Цвета дисплея по умолчанию (RGB565)
uint16_t BGCOLOR_GC = TFT_BLACK; // фон - черный
uint16_t MAINCOLOR_GC = TFT_WHITE; // белок глаза - белый
uint16_t PUPILCOLOR_GC = TFT_BLACK; // зрачок - черный
...
// Конструктор: принимает ссылку на TFT_eSPI
template<typename TFTDisplay>
class RoboEyes
{
private:
  ...
  // Локальная очистка области (теперь не нужна с спрайтом, но оставляем для совместимости)
  void clearEyeArea(int x, int y, int w, int h) {
    if (w <= 0 || h <= 0) return;
    sprite.fillRect(x - 4, y - 4, w + 8, h + 8, backgroundColor); //±4 px — запас
  }

public:
  // Ссылка на объект дисплея TFT_eSPI
  TFTDisplay *display;
  TFT_eSprite sprite = TFT_eSprite(display); // Спрайт для буферизации рисования
  ...
  uint16_t eyeLeftColor = TFT_WHITE; // белый для левого белка
  uint16_t eyeRightColor = TFT_WHITE; // белый для правого белка
  uint16_t pupilLeftColor = TFT_BLACK; // черный для левого зрачка
  uint16_t pupilRightColor = TFT_BLACK; // черный для правого зрачка
  uint16_t backgroundColor = TFT_BLACK; // черный фон
  ...
  // --- предыдущие области для очистки --- (теперь с спрайтом не обязательно, но оставляем)
  ...
  // ОБЩИЕ МЕТОДЫ
  //*********************************************************************************************
  RoboEyes(TFTDisplay &disp) : display(&disp) {};
  ...
  void begin(int width, int height, byte frameRate) {
    ...
    // Инициализация спрайта для буферизации (16-бит цвета для эффективности)
  ...
  void ResetZR() {
    pupilLeftColor = TFT_BLACK;
    pupilRightColor = TFT_BLACK;
  }

  // Сбросить цвета белков на белый
  void ResetGlas() {
    eyeLeftColor = TFT_WHITE;
    eyeRightColor = TFT_WHITE;
  }
  ...
  //*********************************************************************************************
  // РИСОВАНИЕ ГЛАЗ (с использованием спрайта для буферизации)
  //*********************************************************************************************
  void drawEyes() {
    ...
    //// РИСОВАНИЕ В СПРАЙТЕ ////
    // Заполняем спрайт фоном (вместо fillScreen)
    sprite.fillSprite(backgroundColor);

    // Очистка предыдущих областей (опционально, но для плавности)
    if (prev_eyeLx >= 0) {
    ...
    // Рисуем белки глаз в спрайте
    sprite.fillRoundRect(eyeLx, eyeLy, eyeLwidthCurrent, eyeLheightCurrent,
                         eyeLborderRadiusCurrent, eyeLeftColor);
   
    if(!cyclops) {
      sprite.fillRoundRect(eyeRx, eyeRy, eyeRwidthCurrent, eyeRheightCurrent,
                           eyeRborderRadiusCurrent, eyeRightColor);
    }

    // Рисуем зрачки в спрайте
    if(eyeLheightCurrent > 10) {
      ...
      sprite.fillCircle(pupilLx + pupilSizeCurrent/2, pupilLy + pupilSizeCurrent/2,
    ...
    if(!cyclops && eyeRheightCurrent > 10) {
      ...
      sprite.fillCircle(pupilRx + pupilSizeCurrent/2, pupilRy + pupilSizeCurrent/2,
    ...
    // Рисуем уставшие веки в спрайте
    ...
    // Рисуем счастливые веки в спрайте
    ...
    if(sweat) {
      ...
      sprite.fillRoundRect(sweat1XPos, sweat1YPos, sweat1Width, sweat1Height,
      ...
      sprite.fillRoundRect(sweat2XPos, sweat2YPos, sweat2Width, sweat2Height,
                           sweatBorderradius, MAINCOLOR_GC);
      ...
      sprite.fillRoundRect(sweat3XPos, sweat3YPos, sweat3Width, sweat3Height,
                           sweatBorderradius, MAINCOLOR_GC);
    }
    ...
    // Push спрайта на дисплей (без мерцания)
    sprite.pushSprite(0, 0);
  } // конец drawEyes
}; // конец класса RoboEyes
#endif // _FLUXGARAGE_ROBOEYES_GC9A01A_TFT_ESPI_H
